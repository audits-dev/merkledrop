...
contract('MerkleDrop', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    merkleDrop = await MerkleDrop.new(root, token.address, {from: maintainer})
  })

  it('deploys with the Merkle root', async () => {
    const merkleRoot = await merkleDrop.merkleRoot.call()
    assert.equal(merkleRoot, hexRoot)
  })

  it('deploys with the token address', async () => {
    const tokenAddress = await merkleDrop.token.call()
  ...
  describe('claim', () => {
    ...
    it('throws if the contract does not have enough funds', async () => {
      await shouldFail.reverting.withMessage(
        merkleDrop.claim([], [], amounts[0], {from: user1}),
        'Airdrop has insufficient balance')
    })

    context('with funding', () => {
      beforeEach(async () => {
        await token.transfer(merkleDrop.address, totalPayout, {from: maintainer})
      })

      it('throws if the proof and positions are different lengths', async () => {
        await shouldFail.reverting.withMessage(
          merkleDrop.claim([], [1], amounts[0], {from: user1}),
      ...
      context('with a valid proof and amount', () => {
        ...
        it('transfers tokens to the claimer', async () => {
          ...
          await merkleDrop.claim(proof, positions, amounts[0], {from: user1})
        ...
        it('throws if the user tries to claim again', async () => {
          await merkleDrop.claim(proof, positions, amounts[0], {from: user1})
          await shouldFail.reverting.withMessage(
            merkleDrop.claim(proof, positions, amounts[0], {from: user1}),
            'Tokens already claimed')
        })

        it('throws if called by an invalid user', async () => {
          await shouldFail.reverting.withMessage(
            merkleDrop.claim(proof, positions, amounts[0], {from: maintainer}),
            'Invalid proof')
        })
        ...
        it('allows all users to claim', async () => {
          ...
          for (let i = 0; i < addresses.length; i++) {
            ...
            proof = tree.getProof(leaf).map(x => buf2hex(x.data))
            positions = tree.getProof(leaf).map(x => x.position === 'right' ? 1 : 0)
            await merkleDrop.claim(proof, positions, amounts[i], {from: addresses[i]})
          }
          for (let i = 0; i < addresses.length; i++) {
            let balance = await token.balanceOf(addresses[i])
      ...
      context('with an invalid proof', () => {
        ...
        it('throws with message', async () => {
          await shouldFail.reverting.withMessage(
            merkleDrop.claim(proof, positions, amounts[0], {from: user2}),
            'Invalid proof')
        })
      })
